...
docs/                         Per-experiment documentation:
  ...
  runs/00..13                   one doc per experiment, with result tables
  imgs/                         report figures (PNG); regenerated by scripts under
                                kv-eviction-gym/scripts/ (figures.py, figures_e12.py)
kv-eviction-gym/              OUR CONTRIBUTION: the sequential eviction environment
                              (Gymnasium + SB3 MaskablePPO), policies, train/eval
                              scripts, configs for every experiment, and the raw
                              result data (ab_results/, experiments/).
ml-learning-to-evict/         APPLE's repo (KVP), our starting point. Apple's code
                              under its license + ~10 files of ours for the
                              Qwen2-1.5B repro (see the PROVENANCE block in its README).
```

## Attribution: what is Apple's and what is ours
...
```bash
python kv-eviction-gym/scripts/figures.py                                    # figs 1-5, 8-10
python kv-eviction-gym/scripts/figures_e12.py                                # figs 11-14 (E12)
python kv-eviction-gym/experiments/phase3-dataset-causality/make_plots.py    # phase-3 figures
```
